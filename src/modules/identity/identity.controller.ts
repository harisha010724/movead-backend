...
export async function logout(req: Request, res: Response): Promise<void> {
  ...
  res.clearCookie(SESSION_COOKIE[user.audience], sessionCookie(new Date(0)));
  res.status(204).send();
}
...
function setSession(res: Response, issued: identity.SessionIssued): void {
  const { session, token } = issued;
  res.cookie(SESSION_COOKIE[session.audience], token, sessionCookie(session.expiresAt));
}

/**
 * `httpOnly` so an XSS in the portal cannot read it, `SameSite=Strict` so no
 * cross-site request can ride it, and `Secure` outside development — the
 * attributes architecture Part 12.1 requires.
 *
 ...
 * ignore ports.
 */
function sessionCookie(expires: Date): CookieOptions {
